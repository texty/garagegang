...
var startYear = 2016,
  ...
  percents_or_absolutes = $("#percents_or_absolutes")
    .children("option:selected")
    .val(),
  //platform_or_location = "platform",
  platform_or_location =
    platform_type === "Краудфандинг" ? "platform" : "location",
...
d3.csv("data/data_oct_21.csv").then(function (csv) {
  ...
  var platform_list = [
    ...new Set(
      csv
        .filter(function (d) {
          return d.platform_type === "Краудфандинг" && d.status === "Успішний";
        })
        .map(function (d) {
          return d["platform"];
        })
    ...
    if (favorite && favorite.length > 0) {
      dataData = csv.filter(function (d) {
        return (
          d.platform_type === platform_type &&
          d.status === status_type &&
          d.capital != "Економічний" &&
          favorite.includes(d[platform_or_location]) &&
          +d.year >= startYear &&
          +d.year < endYear
        );
      });
  ...
  function change_checkList(value) {
    ...
    for (var li in checkList) {
      $("#checkboxes").append(
        "<label class='platform'><input name='platform' type='checkbox' id='one' value='" +
          checkList[li] +
          "' />" +
          checkList[li] +
          "</label>"
...
function chart(data, xValue, scale, yVal) {
  ...
  platforms.forEach(function (platform) {
    ...
    ["Інфраструктурний", "Культурний", "Людський", "Соціальний"].forEach(
      function (capital) {
  ...
  if (scale === "absolutes") {
    ...
    df.sort(function (a, b) {
      return a["Інфраструктурний"] - b["Інфраструктурний"];
    });
  }
  ...
  var stackedData = d3.stack().keys(subgroups)(df);
  ...
  bars
    ...
    .on("click", function (d) {
      ...
      //малюємо табличку
      table_data = data.filter(function (p) {
        return (
          p[platform_or_location] === d.data.platform &&
          p.capital === clicked_capital
        );
      });
      d3.select(".table-title").html(
        d.data.platform +
          ". проєкти " +
          clicked_capital.replace("ий", "ого") +
          "</span>  капіталу. " +
          startYear +
          " - " +
          (endYear - 1)
